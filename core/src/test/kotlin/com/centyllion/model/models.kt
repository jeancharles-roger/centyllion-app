...
fun dendriteModel(): GrainModel {
    val ms = Grain(id = 0, name = "ms", color = "blue", description = "Manganèse soluble")
    val mc = Grain(id = 1, name ="mc", color = "red", description = "Manganèse cristallisé", movementProbability = 0.0)

    val r1 = Behaviour(
        name = "cristalisation", description = "Cristalisation du Manganèse", probability = 1.0,
        mainReactiveId = mc.id, mainProductId = mc.id, reaction = listOf(Reaction(ms.id, mc.id))
    )

    return GrainModel(name = "Dendrite", description = "test model", grains = listOf(ms, mc), behaviours = listOf(r1))
}
